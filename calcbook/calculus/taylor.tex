...
tangent line -- $f(x+h) \approx f(x) + f'(x) h$ -- and asked two questions: exactly how big is
the error, and can we shrink it by adding a quadratic correction, or a cubic one, and so on?
The chapter's answer, {\it Taylor's Theorem\/}, was reached by a derivation that cheated: it
assumed the various ``$\mu$'' values were all the same. In this appendix we prove the theorem
carefully. Its simplest cases return exactly the tangent line approximation -- but now with a
formula, not a hand wave, for the error.
