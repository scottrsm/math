...
This allows us to more easily compute sums and differences of
...
We do have such a formula; it is the
...
To be honest, the applications below re-derive results we already know -- that
is how we test a new tool; its real power shows on sums that the
linear-combination method above cannot easily crack, such as $\sum n \, 2^n$
-- taken up as an exercise in the next section.
...
\exercise{Find a formula for the sum: $\sum_{n=1}^{N} n \, 2^n$. Hint: note
that $\Delta 2^n = 2^{n+1} - 2^n = 2^n$; so, in the Summation by Parts
formula let $x_n = n$ and $y_n = 2^n$. You will also need the sum
$\sum_{n=1}^{N} 2^{n+1}$, which telescopes for the same reason:
$2^{n+1} = \Delta 2^{n+1}$.}
