...
Here is a question left over from the police work of the last chapter. The
crime lab, double-checking its analysis, asks: how far did the suspect car
actually travel on the day of the robbery? What the lab has on hand is the
velocity record $\{v_n\}$ it computed -- one value for every $1/10^{\rm th}$
of a second, on the order of a million values for the day. Since distance is
velocity times time, the distance traveled over the $n^{\rm th}$ interval is
$v_n\, h$ (with $h = 1/10$), and the total distance is the sum of all these
small distances. A computer can add a million numbers, of course -- the loop
would look much like the find-max program of the last chapter. But a million
additions to produce one number should make us pause: is there a shortcut?

There is, and it comes from remembering where the velocities came from. Each
$v_n$ was computed from a {\it difference\/} of odometer readings,
$v_n = \Delta x_n / h$; so $v_n \, h = \Delta x_n$, where
$\{x_n\}_{n=0}^{N}$ is the odometer data (this time starting the count at
$0$, the morning reading). The distance traveled over the $i^{\rm th}$
interval is $x_{i+1} - x_i$, and the total distance for the day is the sum
of all of these. In mathematical terms, this would be written:
$$
...
And there is the crime lab's shortcut, in full: because the summands are
differences, the million additions collapse to a single subtraction -- the
evening odometer reading minus the morning one, $x_N - x_0$.%
\footnote{\kern 0.5pt \raise 0.5ex \hbox{\dag}}%
{Strictly speaking, the lab holds the ``fuzzed'' disk data of chapter 1,
$d_n = x_n f_n$, not the odometer data itself; each velocity was recovered
as $v_n \approx \Delta d_n / (F\, h)$. The shortcut is the same: the million
additions collapse to the single subtraction $(d_N - d_0)/F$, which
approximates the true difference $x_N - x_0$.}
...
Consider the sum $\sum_{n=1}^{N} n \, 2^n$. The linear-combination method
of the Applications section is of no help here: that method hunted for an anti-difference of $n^2$
...
Now for the payoff -- the sum that opened the Summation by Parts section,
the one the linear-combination method could not touch.
