...
This is the {\it exploit linearity\/} move played for the third time -- first
with $\Delta$ and $\sum$, then with the derivative, and now with the integral:
break the function into pieces, integrate each piece, and add the answers.
...
Consider the integral $\int_0^{\pi/2} x \sin(x) \, dx$. Linearity is no help
here -- the integrand is a {\it product\/}, not a linear combination of
functions we can already integrate. We faced exactly this predicament on the
discrete board: the sum $\sum n \, 2^n$, a product of two very different
sequences, defeated the linear-combination method, and Summation by Parts --
built from the Discrete Product Rule -- rescued us. We now build its
continuous twin the same way, from
the \index{product rule} of \index{differential calculus}. The product rule states
...
This is the integral that opened this subsection; the new tool cracks it.
