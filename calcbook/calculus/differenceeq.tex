...
outstanding debt as 
of the beginning of that month. The rate of interest is given as an annual percentage rate (APR)
and so must be divided by $12$ to get the monthly interest rate,
$r = {\rm APR}/12$. At the beginning (month zero) the outstanding 
...
$$
For this to work, the left hand side itself need not be a telescoping
difference -- it is enough that some known {\it multiple\/} of it is.
So we play the {\it change the representation\/} move: find a known
sequence $\{y_n\}_{n=0}^\infty$ so that, after multiplying our equation
through by a suitable term of $y$, the left hand side becomes the
difference of a product,
$$
\Delta (y_n x_n) = y_{n+1} x_{n+1} - y_n x_n
$$
-- the very quantity whose expansion is the Discrete Product Rule of
chapter 1. Now compare the shape we want with the shape we have:
$$
\obrace{y_{n+1} x_{n+1} - y_n x_n}{what we want}
\qquad {\rm versus} \qquad
\obrace{x_{n+1} - a\, x_n}{what we have}
$$
The leading terms tell us which multiplier to use: multiplying our
equation by $y_{n+1}$ makes the leading terms agree. This gives:
$$
\eqalign{
...
$$
Comparing the trailing terms then forces the choice of the sequence $y$: the left
hand side is {\it exactly\/} the telescoping difference $\Delta(y_n x_n)$ provided
$$
a\, y_{n+1} = y_n
$$
Notice that nothing was pulled out of a hat. The multiplier $y_{n+1}$ and
the condition on $y$ were both {\it forced\/} on us by matching our equation
against the one difference-of-a-product formula we own. With this choice of $y$ we have
...
The unknowns now sit under a telescoping difference, and summing is its
inverse -- the {\it isolate and invert\/} move finishes the job.
...
The graph of the solution over the full $30$ years tells the story at a
glance.
\epsfigure{eps/loanbalance.eps}{Outstanding debt $x_N$ for the loan of the
example ($L = \$100{,}000$, $r = 0.005$, $p = \$600$). The dashed line shows
what a straight-line payoff would look like. Notice how slowly the debt falls
in the early years: at first nearly all of each payment goes to interest.}
...
In fact, this whole chapter was first year algebra replayed with a new kind
of unknown -- the same game on a bigger board. Here is the dictionary:
\bigskip
\centerline{\vbox{\halign{\quad\hfil#\hfil\quad&\quad\hfil#\hfil\quad\cr
{\bf Algebra}&{\bf Difference Equations}\cr
\noalign{\smallskip\hrule\smallskip}
unknown is a number $x$&unknown is a sequence $\{x_n\}$\cr
find an equation $x$ satisfies&find an equation $\{x_n\}$ satisfies\cr
do the same to both sides (balance scale)&multiply both sides by a summing factor\cr
undo multiplication by dividing&undo $\Delta$ by summing (the FTDC)\cr
isolate $x$; a number remains&isolate $x_N$; a formula remains\cr
}}}
\bigskip
The strategic moves were the ones we have played since algebra:
{\it change the representation\/} (make the left hand side a telescoping
difference), then {\it isolate and invert\/} (sum away the difference).
...
How might one solve it? Recall that earlier in this chapter the first order
equation $x_{n+1} = a\, x_n$ had a {\it geometric\/} solution: $x_n = a^n L$.
A second order equation is the same game on a bigger board, so we try the
same {\it shape\/} of solution -- a change of representation: try
a solution of the form $x_n = \lambda^n$, where the number $\lambda$ is left
to be determined. Substituting into the recurrence and
