...
you have a set of input candidates for the maximum.%
\footnote{\kern 0.5pt \raise 0.5ex \hbox{*}}{We are quietly using a fact
about continuous functions: a continuous function on a closed interval
$[a,b]$ actually {\it attains\/} a maximum value somewhere on the interval.
For the nice functions of this book this may be taken on faith.}
You need
...
a single number. This is the {\it change the representation\/} move of the
preface: we trade a picture for a number we can compute with.
This is important enough to state formally.

\proclaim Theorem(Slope Representation). The steepness of a non-vertical
\index{straight line} is captured by a {\it single\/} number - its \index{slope}.
The line itself is pinned down by its slope together with any one of its points.

{\bf proof:\/} From the appendix on straight line functions we see that
a non-vertical line with slope $m$ and $y$-intercept $b$ is the graph
of the function $f(x) = m\,x + b$. Two lines with the same slope $m$ differ
only in $b$ -- a vertical shift -- so they have the same steepness everywhere,
and knowing any one point on the line determines the shift $b$.
...
passes through constants and sums. In the language of the preface, this is
the {\it exploit linearity\/} move on the continuous board: to differentiate
a complicated function, break it into pieces you can differentiate, and add
the answers.
...
and $h'(x)$. A careful proof, however, must handle a subtlety this argument
hides -- the inner change $\Delta u$ can be zero even when $\Delta x$ is
not, making the ratio $\Delta y / \Delta u$ meaningless -- and is beyond
...
The quotient rule need not be taken on faith either -- it is the product and
chain rules in disguise, and the reader owns every piece needed to unmask it.
Write the quotient as a product: $f(x) = g(x)/h(x) = g(x)\, h(x)^{-1}$. The
inner function of $h(x)^{-1}$ is $h$ and the outer function is the power
function $z^{-1}$, whose derivative, by the power rule, is $-z^{-2}$. So the
chain rule gives
$$
\bigl(h(x)^{-1}\bigr)' = -h(x)^{-2}\, h'(x) = -\,{h'(x) \over h^2(x)}
$$
and then the product rule finishes the job:
$$
f'(x) = g'(x)\, h(x)^{-1} + g(x) \biggl(-{h'(x) \over h^2(x)}\biggr)
      = {g'(x) h(x) - g(x) h'(x) \over h^2(x)}
$$
which is exactly the quotient rule stated above. (This is the {\it change
the representation\/} move in miniature: rewrite the quotient as a product,
and tools we already trust take over.)
...
\index{trigonometric function}s. Along the way the next subsections help to
explain the number $e$ that you may have heard of (the story of the angle
measure called \index{radian}s is told alongside the trigonometric limits in
the appendix ``Infinite Sequences and Limits'').
The {\it derivations\/} in the next two subsections may be skimmed on a
first reading without harm to the essential aspects of Calculus; the
{\it results\/}, however, will be used -- the log and exponential functions
in the chapter on differential equations, and the sine and cosine
derivatives as soon as the chapter on Integral Calculus.
...
$\lim_{n \rightarrow \infty}(1 + 1/n)^n$ -- the reduction to this special
sequence is a step we take on faith here. That this special limit {\it
exists\/} is shown in the section ``An Application: The Number $e$'' of the
appendix ``Infinite Sequences and Limits'', by way of the Monotone Sequence
Theorem. Its limiting
value
...
Why should the trigonometric functions join our base collection? Because
they are the functions that {\it oscillate\/} -- and nature is full of
oscillation: a weight bobbing on a spring, a pendulum swinging, alternating
current in a wire. At the close of the chapter on differential equations we
will meet the equation of the swinging pendulum, $x'' + x = 0$, and check
that sine and cosine are the functions that solve it; to be ready for that
we need their derivatives now.
...
$\frac{\cos(h_n) - 1}{h_n}$. The picture below shows what is at stake in the first of these limits. When
the angle $\theta$ is small, the vertical segment BC (whose length is
$\sin(\theta)$) and the arc DC (whose length is $\theta$, in radian measure)
hug each other. In fact, one might conclude that in the limit their ratio
$\frac{\sin(\theta)}{\theta}$ is $1$.
\epsfigure{eps/sinderiv.eps}{Comparison of triangle ABC, sector ADC,
and triangle ADE.}
This conclusion is correct. The precise argument -- comparing the areas of
the triangles ABC and ADE with the area of the sector ADC and squeezing --
is a fine application of the Squeeze Limit Theorem, and it may be found,
along with a discussion of \index{radian} measure, in the section ``Two
Trigonometric Limits'' of the appendix ``Infinite Sequences and Limits''.
The radian is to angles what the base $e$ was to logarithms: precisely the
choice of measure that makes these limits -- and with them the derivative
formulas below -- come out clean, with no messy constants tagging along.
The second limit follows from the first (also shown in the appendix).
The results are:
$$
