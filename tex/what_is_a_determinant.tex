...
vectors: $\{ [1, 0, \ldots, 0], [0, 1, 0, \ldots, 0], \ldots, [0, \ldots, 0, 1]\}$.
...
indices. We denote the parity of $\sigma$ by ${\rm sgn}(\sigma)$:%
\footnote{The mapping ${\rm sgn}: S_n \rightarrow {\bf Z}_2$ is a group homomorphism.
Throughout this paper ${\rm sgn}(\sigma) \in \{0, 1\}$ is the {\it parity\/} of $\sigma$;
the actual sign $\pm 1$ is then $(-1)^{{\rm sgn}(\sigma)}$. (Some texts define
${\rm sgn}(\sigma) \in \{-1, +1\}$ directly.)}
${\rm sgn}(\sigma) = 1$ if $k$ is odd and ${\rm sgn}(\sigma) = 0$
...
\end{align}
For instance, after rearranging the multiplications in \eqref{det_formula_sinv}
...
Similarly, the next term is $a_{2, \sigma(2)}$, etc. Then \eqref{det_formula_sinv2}
can be written as:
\begin{align}
  |A| &= \sum_{\sigma \in S_n} (-1)^{{\rm sgn}(\sigma^{-1})} a_{\sigma(1),1}^\prime
            a_{\sigma(2),2}^\prime \, \ldots \, a_{\sigma(n),n}^\prime \label{at_proto}
\end{align}
Here, we define $a_{i,j}^\prime$ as the $i^{\rm th}$, $j^{\rm th}$ element of $A^T$.
...
the mapping ${\rm sgn}: S_n \rightarrow {\bf Z}_2$
...
or both be $1$ for their sum to be $0$ in ${\bf Z}_2$.}
...
$|{\boldsymbol \cdot}| : R^{n \times n} \rightarrow R$.
...
To see this, consider any non-identity permutation $\sigma$. Then for some index $i$,
$\sigma(i) = j \ne i$, so $(I+H)_{\sigma(i), i} = (I+H)_{j, i} = H_{j, i}$ (off-diagonal).
Now $\sigma(j)$ cannot equal $j$, because $i$ is already sent there
(permutations are one-to-one mappings).
Therefore, for this permutation, $\sigma$, the product term contains the product of
two off-diagonal entries of $H$ times the other $n-2$ entries
($H_{i,j} \, H_{j,k} \cdot (\text{product of $n-2$ other terms})$), which is $o(H)$.
...
{\bf Question:\/} How is the volume of the parallelepiped formed from the vectors $\{{\bf x}_{0,i}\}_{i=1}^n$
evolving in time?
...
$|X(t)| = |X_0|\,e^{{\rm tr}(A)\,t}$,
the volume is preserved exactly when ${\rm tr}(A) = 0$. An important
sub-class is anti-symmetric matrices: when $A$ is {\em anti-symmetric\/}
($A^T = -A$), it follows that $\left< A {\bf x}, {\bf x} \right> = 0$.
