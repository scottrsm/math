...
\end{align*} 
Notice that we could pull out $\{a_{i,j}\}_{i,j=1}^n$
...
\begin{align}
  |A + H| &= |A\left(I + A^{-1}H\right)| \nonumber \\
          &= |A|\, |I + A^{-1}H|
                \quad \text{(Multiplicative Property:
                $|AB| = |A|\,|B|$)} \nonumber \\
...
on phase-space volume preservation in dynamical systems. The chain-rule step above
used $X(t)^{-1}$; this is justified post hoc, since $|X(t)| = |X_0|\, e^{{\rm tr}(A)\, t}$
is non-zero whenever $|X_0| \neq 0$, so $X(t)$ remains invertible along the flow.
...
the volume of the parallelepiped they span.

\paragraph{Exercise.} In this case, what kind of matrix is $X(t)$?
