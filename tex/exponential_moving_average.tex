...
The {\em half-life\/} is the number of steps $H$ such that $w_H / w_1 = 1/2$.
Substituting $w_n = \alpha \lambda^{n-1}$ gives:
\begin{eqnarray}
	\frac{w_H}{w_1} &=& \frac{\alpha \lambda^{H-1}}{\alpha} = \lambda^{H-1} = \frac{1}{2}
\end{eqnarray}
For a given $\lambda$, the integer $H$ satisfying this exactly may not exist;
in practice we use the closest integer.\footnote{Note that $H$ must be greater than $1$
...
$\{0\} \cup \{2, 3, \ldots, k\}$ — yielding an inner sum that no longer depends on $i$ —
and subtract back the $k_i \in \{2, \ldots, k\}$ contributions as a separate $z$-indexed
sum, with $z = k_i$. The multinomial coefficient with one slot equal to $z$ factors as
...
Which is
\begin{align}
    \E{(x_i - {\bar x})^3 w_i} &= M_3 + M_3 \left(-3 \sum_{i=1}^N w_i^2 + 3 \sum_{i=1}^N w_i^3 - \sum_{i=1}^N w_i^4\right) 
...
The corresponding relation for the relative $4^{\it th}$ moment is:
\begin{align}
...
$\sum_{i=1}^N (x_i - {\bar x})^4 w_i$. Our estimate of the relative kurtosis is then:
\begin{align}
