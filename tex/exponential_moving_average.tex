...
\begin{flalign}
    E\left[ \sum_{i=1}^N (x_i - {\bar x})^r w_i \right] &=  \sum_{i=1}^N E\left[(x_i - {\bar x})^r\right] w_i && \\\nonumber 
    &=  \sum_{i=1}^N E\left[\left((x_i - \mu) + (\mu - {\bar x})\right)^r\right] w_i && \\\nonumber 
    &=  \sum_{i=1}^N E\left[\sum_{j=0}^r (-1)^j \binom{r}{j} (x_i - \mu)^{r-j}({\bar x} - \mu)^{j}\right] w_i  && \\\nonumber
    &=  \sum_{i=1}^N \sum_{j=0}^r (-1)^j \binom{r}{j} E\left[(x_i - \mu)^{r-j}({\bar x} - \mu)^{j}\right] w_i  &&\\\nonumber
    &=  \sum_{i=1}^N \sum_{j=0}^r (-1)^j \binom{r}{j} E\left[(x_i - \mu)^{r-j}\left(\sum_{l=1}^n (x_l - \mu)w_l\right)^j\right] w_i &&\\\nonumber 
    &=  \sum_{i=1}^N \sum_{j=0}^r (-1)^j \binom{r}{j} E\left[(x_i - \mu)^{r-j}\left((x_i - \mu)w_i + \sum_{\substack{l=1 \\ l \neq i}}^N (x_l - \mu)w_l\right)^j\right] w_i &&\\\nonumber 
    &=  \sum_{i=1}^N \sum_{j=0}^r (-1)^j \binom{r}{j} E\left[ (x_i - \mu)^{r-j}
    \sum_{k=0}^j \binom{j}{k} (x_i - \mu)^{j-k} w_i^{j-k}  \left(\sum_{\substack{l=1 \\ l \neq i}}^N (x_l - \mu)w_l\right)^k\right] w_i &&\\\nonumber 
    &=  \sum_{i=1}^N \sum_{j=0}^r \sum_{k=0}^j (-1)^j \binom{r}{j} \binom{j}{k} E\left[(x_i - \mu)^{r-k}w_i^{j-k}
     \left(\sum_{\substack{l=1 \\ l \neq i}}^N (x_l - \mu)w_l\right)^k\right] w_i &&\\\nonumber 
    &=  \sum_{i=1}^N \sum_{j=0}^r \sum_{k=0}^j (-1)^j \binom{r}{j} \binom{j}{k} E\left[(x_i - \mu)^{r-k} \right]
     E\left[\left(\sum_{\substack{l=1 \\ l \neq i}}^N (x_l - \mu)w_l\right)^k\right] w_i^{j+1-k} &&\\\nonumber 
    &=  \sum_{i=1}^N \sum_{j=0}^r  (-1)^j \binom{r}{j} E\left[(x_i - \mu)^{r} \right] w_i^{j+1} &&\\\nonumber 
    & \quad + \sum_{i=1}^N \sum_{j=2}^r \sum_{k=2}^j (-1)^j \binom{r}{j} \binom{j}{k} E\left[(x_i - \mu)^{r-k} \right]
     E\left[\left(\sum_{\substack{l=1 \\ l \neq i}}^N (x_l - \mu)w_l\right)^k\right] w_i^{j+1-k} &&\\\nonumber  &&
\end{flalign}

Consequently,
\begin{flalign}
    E\left[ \sum_{i=1}^N (x_i - {\bar x})^r w_i \right] &=  M_r + M_r \sum_{j=1}^r  (-1)^j \binom{r}{j} \sum_{i=1}^N w_i^{j+1} 
    ...
    + \sum_{i=1}^N \sum_{j=2}^r \sum_{k=2}^j (-1)^j \binom{r}{j} \binom{j}{k} M_{r-k} &&\\\nonumber
    & \quad  \times  \sum_{\substack{k_1 + k_2 \ldots + k_N = k \\ \forall n, k_n \ge 0 \, \wedge \,k_n \neq 1 \\ k_i = 0}} \binom{k}{k_1, k_2, \ldots, k_N} 
    \left(\prod_{\substack{l=1 \\ l \neq i}}^N E\left[(x_l - \mu)^{k_l}\right] w_l^{k_l}\right) w_i^{j+1-k} &&\\\nonumber 
    &=  M_r + M_r \sum_{j=1}^r  (-1)^j \binom{r}{j} \sum_{i=1}^N w_i^{j+1} 
    + \sum_{i=1}^N \sum_{j=2}^r \sum_{k=2}^j (-1)^j \binom{r}{j} \binom{j}{k} M_{r-k} &&\\\nonumber
    & \quad  \times  \sum_{\substack{k_1 + k_2 \ldots + k_N = k \\ \forall n, k_n \ge 0 \, \wedge \,k_n \neq 1 \\ k_i = 0}} \binom{k}{k_1, k_2, \ldots, k_N} 
...
\begin{flalign}
    E\left[ (x_i - {\bar x})^r w_i \right] &= M_r + M_r \sum_{j=1}^r  (-1)^j \binom{r}{j} \sum_{i=1}^N w_i^{j+1} 
    + \sum_{j=2}^r \sum_{k=2}^j (-1)^j \binom{r}{j} \binom{j}{k} M_{r-k} && \\\nonumber 
     ...
     \binom{k-z}{k_1, k_2, \ldots, k_{N-1}} \left(\prod_{\substack{l=1 \\ l \neq i}}^{N-1} M_{k_l} w_l^{k_l} \right) w_i^{j+z+1-k} \right] &&
\end{flalign}
We check the case $r=2$ to see if it coincides with the previous section:

\begin{flalign}
    E\left[ (x_i - {\bar x})^2 w_i \right] &= \sigma^2 + \sigma^2 \sum_{j=1}^2  (-1)^j \binom{2}{j} \sum_{i=1}^N w_i^{j+1} 
...
\begin{flalign}
    E\left[ (x_i - {\bar x})^2 w_i \right] &= \sigma^2 + \sigma^2 \sum_{j=1}^2  (-1)^j \binom{2}{j} \sum_{i=1}^N w_i^{j+1} &&\\\nonumber 
     & + \sum_{\substack{k_1 + k_2 \ldots + k_N = 2 \\ \forall n, k_n \ge 0 \, \wedge \, k_n \neq 1 }} \binom{2}{k_1, k_2, \ldots, k_N} 
     \left(\prod_{l=1}^N M_{k_l} w_l^{k_l}\right) \left(\sum_{i=1}^N w_i \right)  - \sigma^2  \left(\sum_{i=1}^N w_i^{3} \right) &&
\end{flalign}
Which is
\begin{flalign}
    E\left[ (x_i - {\bar x})^2 w_i \right] &= \sigma^2 + \sigma^2 \left( -2\sum_{i=1}^N w_i^2 + \sum_{i=1}^N w_i^3\right) 
     +  \sigma^2 \sum_{l=1}^N w_l^2\left(\sum_{i=1}^N w_i\right) - \sigma^2  \left(\sum_{i=1}^N w_i^{3} \right) &&\\\nonumber
\end{flalign}
This becomes
\begin{flalign}
    E\left[ (x_i - {\bar x})^2 w_i \right] &= \sigma^2\left(1 - \sum_{i=1}^N w_i^2\right) &&
\end{flalign}

As a more complicated example we try $r=3$. In this case the formula is:
\begin{flalign}
    E\left[ (x_i - {\bar x})^3 w_i \right] &= M_3 + M_3 \sum_{j=1}^3  (-1)^j \binom{3}{j} \sum_{i=1}^N w_i^{j+1} 
...
Which is
\begin{flalign}
    E\left[ (x_i - {\bar x})^3 w_i \right] &= M_3 + M_3 \left(-3 \sum_{i=1}^2 w_i^2 + 3 \sum_{i=1}^N w_i^3 - \sum_{i=1}^N w_i^4\right) 
     - M_3 \left(\sum_{l=1}^N w_l^3 \left(\sum_{i=1}^N w_i\right) - \sum_{i=1}^N w_i^4 \right) &&  
\end{flalign}

Or,
\begin{flalign}
    E\left[ (x_i - {\bar x})^3 w_i \right] &= M_3\left(1 - 3\sum_{i=1}^N w_i^2 + 2\sum_{i=1}^N w_i^3\right) && 
\end{flalign}
...
\begin{flalign}
    E\left[ (x_i - {\bar x})^4 w_i \right] &= M_4 + M_4 \sum_{j=1}^4  (-1)^j \binom{4}{j} \sum_{i=1}^N w_i^{j+1} 
...
This is
\begin{flalign}
    E\left[ (x_i - {\bar x})^4 w_i \right] &= M_4\left(1 -4 \sum_{i=1}^N w_i^2 + 6 \sum_{i=1}^N w_i^3 -4 \sum_{i=1}^N w_i^4 + \sum_{i=1}^N w_i^5 \right) &&\\\nonumber
    ...
    &- M_2 \sum_{i=1}^N \left( \sum_{\substack{l=1 \\ l \neq i}}^N M_2 w_l^2\right)  w_i^3  - M_4 \sum_{i=1}^N w_i^5  &&\\\nonumber 
\end{flalign}

\begin{flalign}
    E\left[ (x_i - {\bar x})^4 w_i \right] &= M_4\left(1 -3 \sum_{i=1}^N w_i^2 + 6 \sum_{i=1}^N w_i^3 - 3 \sum_{i=1}^N w_i^4 
    ...
    &- M_2^2 \sum_{i=1}^N \left( \sum_{\substack{l=1 \\ l \neq i}}^N w_l^2\right) w_i^3 &&\\\nonumber 
\end{flalign}

Let $W_j = \sum_{i=1}^N w_i^j$ for $j \in [1,5]$. We may then write the above as:
\begin{flalign}
    E\left[ (x_i - {\bar x})^4 w_i \right] &= M_4\left(1 - 3W_2 + 6 W_3 -3W_4 \right) &&\\\nonumber
    &+  6M_2^2 W_2 -12 M_2^2 W_2^2 + 12M_2^2W_2 W_4 - 6M_2^2 W_2 W_5  &&\\\nonumber 
    &+ 6M_2^2 \sum_{i=1}^{N-1} \sum_{m=1+1}^N w_l^2 w_m^2 - M_2^2 \sum_{i=1}^N \sum_{\substack{l=1 \\l \neq i}}^N w_l^2 w_i^3 
\end{flalign}
...
\begin{flalign}
    E\left[ (x_i - {\bar x})^4 w_i \right] &= M_4\left(1 - 3W_2 + 6 W_3 -3W_4 \right) &&\\\nonumber
    &+  6M_2^2 W_2 -12 M_2^2 W_2^2 + 12M_2^2W_2 W_4 - 6M_2^2 W_2 W_5  &&\\\nonumber 
    &+ 6M_2^2 \sum_{i=1}^{N-1} \sum_{m=1+1}^N w_l^2 w_m^2 - M_2^2 W_2 W_3  + M_2^2 W_5
\end{flalign}

There relative $4^{\it th}$ na\"{i}ve moment becomes:
\begin{flalign}
    \frac{E\left[ (x_i - {\bar x})^4 w_i \right]}{M_2^2} &= \frac{M_4}{M_2^2} \left(1 - 3W_2 + 6 W_3 -3W_4 \right) &&&&\\\nonumber
    &+  6 W_2 -12 W_2^2 + 12W_2 W_4 - 6 W_2 W_5  &&\\\nonumber 
    &+ 6 \sum_{i=1}^{N-1} \sum_{m=1+1}^N w_l^2 w_m^2 - W_2 W_3  + W_5
\end{flalign}
Therefore, the estimate for the unbiased relative $4^{\it th}$ moment is:
\begin{flalign}
    \frac{M_4}{M_2^2} &= \frac{\frac{E\left[ (x_i - {\bar x})^4 w_i \right]}{M_2^2} 
    -  6 W_2 +12 W_2^2 - 12W_2 W_4 + 6 W_2 W_5  
    - 6 \sum_{i=1}^{N-1} \sum_{m=1+1}^N w_l^2 w_m^2 + W_2 W_3  - W_5}{\left(1 - 3W_2 + 6 W_3 -3W_4 \right)}
\end{flalign}
...
\begin{flalign}
    \frac{M_4}{M_2^2} &= \frac{\frac{(1 - W_2)^2 \sum_{i=1}^N (x_i - {\bar x})^4 w_i }{\sum_{i=1}^N  \left(x_i - {\bar x})^2 w_i\right)^2  } 
    -  6 W_2 +12 W_2^2 - 12W_2 W_4 + 6 W_2 W_5  
    - 6 \sum_{i=1}^{N-1} \sum_{m=1+1}^N w_l^2 w_m^2 + W_2 W_3  - W_5}{\left(1 - 3W_2 + 6 W_3 -3W_4 \right)}
\end{flalign}
