...
\begin{flalign}
    ...
    + \sum_{i=1}^N \sum_{j=2}^r \sum_{k=2}^j (-1)^j \binom{r}{j} \binom{j}{k} M_{r-k} &&\\\nonumber
    & \quad  \times  \sum_{\substack{k_1 + k_2 \ldots + k_N = k \\ \forall n, k_n \ge 0 \\ \forall n, n \neq i \wedge k_n \neq 1 \\ k_i = 0}} \binom{k}{k_1, k_2, \ldots, k_N} 
...
Finally, we have the formula for the expected value of the na\"{i}ve $r^{\rm th}$ moment:
\begin{flalign}
    E\left[ \sum_{i=1}^N (x_i - {\bar x})^r w_i \right] &= M_r + M_r \sum_{j=1}^r  (-1)^j \binom{r}{j} \sum_{i=1}^N w_i^{j+1} &&\\\nonumber   
    & + \sum_{i=1}^N \sum_{j=2}^r \sum_{k=2}^j (-1)^j \binom{r}{j} \binom{j}{k} M_{r-k} 
     \sum_{\substack{k_1 + k_2 \ldots + k_N = k \\ \forall n, k_n \ge 0 \\ \forall n, n \neq i \wedge k_n \neq 1 \\ k_i = 0}} \binom{k}{k_1, k_2, \ldots, k_N} 
...
\begin{flalign}
    ...
    & \quad + \sum_{i=1}^N \left( 
     \sum_{\substack{k_1 + k_2 \ldots + k_N = 2 \\ \forall n, k_n \ge 0 \\ \forall n, n \neq i \wedge k_n \neq 1 \\ k_i = 0}} \binom{2}{k_1, k_2, \ldots, k_N} 
     \left(\prod_{l=1}^N M_{k_l} w_l^{k_l}\right) w_i^{j+1-k} \right) &&
\end{flalign}
In the multinomial sum with the given constraints, we see that the only combinations of $k$'s that work are when 
one and only one of the $k$'s
has the value two (except when $k_l = k_i$) while all other values are $0$.
Therefore,
...
to render the na\"{i}ve $r^{\rm th}$ moment unbiased.
