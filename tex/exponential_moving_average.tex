...
\begin{flalign}
    ...
    &=  \sum_{i=1}^N \sum_{j=0}^r (-1)^j \binom{r}{j} E\left[(x_i - \mu)^{r-j}\left((x_i - \mu)w_i + \sum_{\substack{l \in [1, N] \\ l \neq i}} (x_l - \mu)w_l\right)^j\right] w_i &&\\\nonumber 
...
\begin{flalign}
    ...
    \left(\prod_{\substack{l \in [1,N] \\ l \neq i}} E\left[(x_l - \mu)^{k_l}\right] w_l^{k_l}\right) w_i^{j+1-k} &&\\\nonumber 
    &=  M_r + M_r \sum_{j=1}^r  (-1)^j \binom{r}{j} \sum_{i=1}^N w_i^{j+1} 
    + \sum_{i=1}^N \sum_{j=2}^r \sum_{k=2}^j (-1)^j \binom{r}{j} \binom{j}{k} M_{r-k} &&\\\nonumber
    & \quad  \times  \sum_{\substack{k_1 + k_2 + \, \ldots\,  + k_N = k \\ \forall n, k_n \ge 0 \, \wedge \,k_n \neq 1 \\ k_i = 0}} \binom{k}{k_1, k_2, \, \ldots\, , k_N} 
