...
analog of continuous exponential decay. To see this let $f(t) = e^{-kt}$ 
...
By the half-life of $\lambda$, we mean the number of powers of $\lambda$, $H$, so that
...
\begin{flalign}
    ...
    + \sum_{j=2}^r \sum_{k=2}^j (-1)^j \binom{r}{j} \binom{j}{k} M_{r-k} && \\\nonumber 
     ...
     &\left. - \sum_{z = 2}^k M_z \left( \sum_{i=1}^N w_i^{j+z+1-k} \sum_{\substack{k_1 + k_2 + \, \cdots\, + k_{N-1} = k-z \\ \forall n, k_n \ge 0\, \wedge\, k_n \neq 1}}
...
\begin{flalign}
    ...
    &+ \binom{4}{2}\binom{2}{2} M_2^2 \left( \sum_{l=1}^N w_l^2 \left(\sum_{i=1}^N w_i\right)   - \sum_{l=1}^N w_l^2\left(\sum_{i=1}^N w_i^3 \right)\right) && \\\nonumber
    &- \binom{4}{3}\binom{3}{2} M_2^2 \left( \sum_{l=1}^N w_l^2 \left(\sum_{i=1}^N w_i^2\right) - \sum_{l=1}^N w_l^2\left(\sum_{i=1}^N w_i^4 \right)\right) && \\\nonumber
    &+ \binom{4}{4}\binom{4}{2} M_2^2 \left( \sum_{l=1}^N w_l^2 \left(\sum_{i=1}^N w_i^3\right) - \sum_{l=1}^N w_l^2\left(\sum_{i=1}^N w_i^5 \right)\right) && \\\nonumber
...
Since we don't know $M_2 = \sigma^2$, we use our unbiased estimate: $\frac{\sum_{j=1}^N (x_j - {\bar x})^2 w_j}{1 - \sum_{j=1}^N w_j^2}$.
