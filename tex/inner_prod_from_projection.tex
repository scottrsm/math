...
original problem. Instead of starting at a point, $p_0$, we are starting at 
...
= {\bf w} \cdot {\bf v} = 0$, so $P_{\bf w}({\bf v}) = 0$ as well: there
...
{\bf Def.} An {\em orthonormal\/} set is a collection of $m$ vectors
...
{\bf Def.} We define $P_{\mathrm{span}\{{\bf v}_1, \ldots, {\bf v}_m\}}$ to be
the projection onto the set of points {\em spanned\/} (swept out) by the
vectors ${\bf v}_1, \ldots, {\bf v}_m$.

Formula \eqref{proj2} can generalize this so that if
...
\be
  P_{\mathrm{span}\{{\bf v}_1, \ldots, {\bf v}_m\}}({\bf x}) & = & \sum_{i=1}^m ( {\bf x} \cdot {\bf v}_i) {\bf v}_i \label{projection}
\ee
...
$\langle{\bf x}, {\bf y}\rangle$. An inner product is any function of two vectors; i.e., $F: V \times V \rightarrow {\bf R}$, with the following properties:
\bi
...
$$L^2 = \{ {\bf f} \mid \text{$f$ is integrable with} \int_\Omega f(x)^2 \, dx < \infty\}.$$
