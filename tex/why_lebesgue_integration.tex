...
Suppose the sequence of functions, $f_n$, converges point-wise to a function $f$.
We wish to find minimal conditions to place on $f_n$ so that the following occurs:

{\bf Theorem?}
\begin{equation}
	\lim_{n \to \infty} \int f_n(x) \, dx = \int \lim_{n \to \infty} f_n(x) \, dx \label{theorem}
\end{equation}

Here is a concrete instance of the question. Every calculus student has
integrated a power series term by term:
\begin{align*}
	\int_0^{1/2} \frac{1}{1-x}\, dx
		&= \int_0^{1/2} \lim_{n \to \infty} \left(1 + x + x^2 + \cdots + x^n\right) dx \\
		&\overset{?}{=} \lim_{n \to \infty} \int_0^{1/2} \left(1 + x + x^2 + \cdots + x^n\right) dx
		 = \frac{1/2}{1} + \frac{(1/2)^2}{2} + \frac{(1/2)^3}{3} + \cdots
\end{align*}
The step marked ``?'' is exactly an application of \eqref{theorem}: a limit
has been interchanged with an integral. In this instance the move is
legitimate -- the partial sums converge uniformly on $[0, 1/2]$ -- and both
sides equal $\ln 2$. But the student who did this never asked permission,
and the question of this paper is: when is the move legal?

Like any good technology this simplifies the work of mathematicians
and applied scientists. For instance, suppose the $f_n$ converge to the function $f$
...
uniformly to $f$ on a closed bounded interval. 
...
times they don't have this strong condition and yet \eqref{theorem}
...
functions which allow \eqref{theorem} to hold.
...
\section{Looking for Better Convergence Criteria}
...
Since we are working with Riemann integration,
...
\begin{align}
	f_n(x) &= \begin{cases}  1 \quad \text{if $x = \frac{p}{q}$ for some integers $p, q$ with $0 \le p \le q$ and $1 \le q \le n$;} \\
...
First, let $\{r_i\}$ be an enumeration of the rationals -- a bijection
between the positive integers and the rationals. We know we can do this as
the rationals are countable. (Only the
rationals in $[0,1]$ matter for the bound below; using a broader enumeration
is harmless because covers in $E_{i,n}$ outside $[0,1]$ contribute zero to
...
The choice of widths is the whole trick: give the $i^{th}$ rational an
interval ``budget'' that halves at each step, so that the total budget,
$\sum_{i=1}^\infty 2/(n 2^{i}) = 2/n$, is finite -- and can be made as small
as we please by increasing $n$. (Keep this halving-budget device in mind; it
will return.)
...
\begin{align}
	g_{n}(x) &= \sum_{i=1}^\infty S_{i}(x) I_{E_{i,n}}(x)
\end{align}
...
\begin{align}
	\int_0^1 S_{i}(x) I_{E_{i,n}}(x)\, dx \le \int_{0}^1 I_{E_{i,n}}(x) \, dx &\le \frac{2}{n2^{i}} \nonumber \\
	\int_0^1 f(x) \, dx = \int_0^1 g_n(x) \, dx &= \int_0^1 \sum_{i=1}^\infty  S_{i}(x) I_{E_{i,n}}(x)\, dx  \label{sum-limit-exchange} \\
												&\le  \sum_{i=1}^\infty \int_0^1   S_{i}(x) I_{E_{i,n}}(x)\, dx  \nonumber \\
												&\le  \sum_{i=1}^\infty \frac{2}{n2^{i}} \nonumber \\
												&=  \frac{2}{n} \sum_{i=1}^\infty \frac{1}{2^{i}} \nonumber \\
												&=  \frac{2}{n} \nonumber
 \end{align}

For the sake of the argument above -- to get an upper bound on the integral of $f$ -- we are assuming that
$f(x)$ (and consequently, $g_n(x)$) is Riemann integrable and that we
can exchange the integral and sum in equation \eqref{sum-limit-exchange}.
(Strictly, the interchange would require uniform convergence or a Lebesgue-style
dominated/monotone result -- neither of which we have here. The point of the
heuristic is to show what value the integral of $f$ would have to take if it
existed.)

What does this say about the integral of $f$? 
Given any $n \in {\bf Z}^+$, we can bound the integral of $f$
...
Since this is true for all $n \in {\bf Z}^+$, the integral of $f$ must be $\le 0$.
...
$0$ and $1$ respectively. To see this, note that 
over any sub-interval of the partition, the minimum (infimum) value is always 0 and
...
\footnote{A classical example: there are {\em continuous\/} functions
decreasing pointwise to the characteristic function of a ``fat Cantor set''
-- a closed set with positive length and empty interior. That characteristic
function is discontinuous at every point of the set -- a set of positive
length -- and so fails Riemann's integrability criterion.}
...
analog of Riemann's step functions: characteristic (indicator) functions of
(possibly complicated) sets:

$\chi_{S}(x) = \begin{cases} 1 \quad \text{if $x \in  S$;} \\ 0 \quad \text{otherwise.} \end{cases}$
...
as a characteristic function: $f(x) = \chi_{Q_{[0,1]}}(x)$.

Here, $Q_{[0,1]}$ is the set of rationals on the interval $[0, 1]$.
...
\begin{align}
	\mu^*(S) &= \inf_{S \subseteq \mathop{\cup}_{i=1}^\infty I_i} \sum_{i=1}^\infty |I_i|
\end{align}

This notion of measure (length) would not be of much use if the Lebesgue outer measure of
an interval was not its length.
It can be shown that it does indeed coincide with length on
intervals -- the lower bound is the honest part of the work, requiring a
compactness (Heine--Borel) argument -- and also on a finite collection of
{\em disjoint\/} intervals (where it equals the sum of lengths).

Well, what is the outer measure of the set $Q_{[0,1]}$? We claim that the ``length'' 
(outer-measure) of the set of rationals on the interval $[0,1]$ is $0$.

Consider the coverings $\left(r_i - \frac{\epsilon}{2^i}, r_i + \frac{\epsilon}{2^i}\right)$
-- the halving budget of the last section again, this time with total
length $2\epsilon$.
Then for any
$\epsilon > 0$, $Q_{[0,1]} \subset \mathop{\cup}_{i=1}^\infty \left(r_i - \frac{\epsilon}{2^i}, r_i + \frac{\epsilon}{2^i}\right)$.
Then $\forall \epsilon > 0$:
\begin{align*}
\mu^*(Q_{[0,1]}) &\le \sum_{i=1}^\infty  \left|\left(r_i - \frac{\epsilon}{2^i}, r_i + \frac{\epsilon}{2^i}\right)\right| \\
		 &= 2 \epsilon \sum_{i=1}^\infty \frac{1}{2^i} \\
		 &= 2 \epsilon
\end{align*}
Consequently, $\mu^*(Q_{[0,1]}) = 0$.

With this we can {\em define\/} the Lebesgue integral of the function:
$f(x) = \chi_{Q_{[0,1]}}(x)$ over the interval $[0,1]$
as $1 \cdot \mu^*(Q_{[0,1]}) = 1 \cdot 0 = 0$.

In general, if a function can be written as a finite linear combination of
characteristic functions: $h(x) = \sum_{i=1}^N c_i \chi_{E_i}(x)$,
then we would {\em define\/} the Lebesgue integral for such functions as:
\begin{align}
	\int h(x) \, dx &= \sum_{i=1}^N c_i \, \mu^*(E_i)
\end{align}
(There is a hidden obligation here: the same function $h$ can be written in
many ways as such a combination, and the definition is only sensible if
every way of writing $h$ gives the same value.  Making this work is another
reason for the restriction to {\em measurable\/} sets, described below.)
...
functions -- ones that are finite linear combinations of characteristic
...
``mu'' method and used the notation $\mu(C)$ to be his measure of the surface
area of an object $C$.
...
the comb was a little more than half the area as the comb's container.
...
The retailer said it was much easier to store, you got a finer comb, and you 
...
teeth of the combs, the oil coating did not penetrate the fine crevices of the
...
Sets like $C$ are too jagged and their measure doesn't add properly.%
\footnote{That such sets exist at all is a famous and non-obvious fact: the
classical example is the {\em Vitali set\/} (1905), and every known
construction requires the Axiom of Choice.  No set you can explicitly write
down will fail to be measurable.}
...
\begin{align}
	\mu^*(X) &= \mu^*(X \cap C) + \mu^*(X \cap C^c) \label{caratheodory}
\end{align}
...
in the sense that the length of a set and the length of its ``complement'' 
must add up to their union. 

But why all the fuss about the intersection with ``any set $X$''.
Why not just say that $C$ and its complement add up properly?
The problem is that the complement of $C$ might have infinite length, because
the ambient space containing $C$ is infinite.
Saying that $C$ and its complement add up correctly is the statement that 
the length of $C$ and the length of its complement is equal to the length 
...
Without some restriction, the complement could be 
...
bounding boxes.%
\footnote{A pleasant fact: for subsets of a bounded interval, testing against
the single box $X = [0,1]$ turns out to be {\em equivalent\/} to
Carath\'eodory's condition for Lebesgue outer measure -- this was, in
essence, Lebesgue's original definition, via inner measure.  The
``all $X$'' form is the one that survives in general, for abstract outer
measures where no natural box exists.}
And now, we see what ``X'' accomplishes in the definition of
measurability, (\ref{caratheodory}). The ``X'' in this definition is a stand-in for 
{\em all\/} bounding boxes; but this includes, in particular, the/any 
bounding boxes that might be considered ``natural''. 
Consequently, (\ref{caratheodory}) provides an unambiguous definition to filter out 
...
measuring method called outer measure to so-called measurable sets via 
the filtering condition, (\ref{caratheodory}).
...
\begin{align*}
	\mu(A) &= \sum_{i=1}^N \mu(A_i) \quad \text{when} \; 
		A = \mathop{\cup}_{i=1}^N A_i \quad \text{with} \;  
		A_i \cap A_j = \emptyset \quad \text{if} \; i \neq j \\
	\mu(A) &= \sum_{i=1}^\infty \mu(A_i) \quad \text{when} \; 
		A = \mathop{\cup}_{i=1}^\infty A_i \quad \text{with} \; 
		A_i \cap A_j = \emptyset \quad \text{if} \; i \neq j
\end{align*}
...
Two of these closure properties can at least be made plausible in a line
each. Complements come for free because Carath\'eodory's condition
\eqref{caratheodory} is {\em symmetric\/} in $C$ and $C^c$ -- swap them and
you get the same equation back. And countable unions are tamed by -- for
the third time in this paper -- the halving-budget device: to fit countably
many small errors under one overall tolerance $\epsilon$, allow the $i^{th}$
set an error of $\epsilon / 2^{i}$.
...
condition, (\ref{caratheodory}), on simple ``base'' sets and then we can claim that 
...
is easy to show satisfy (\ref{caratheodory}), and yet provide a wealth of complex sets
via set operations.
...
{\em simple\/} functions that is monotonically increasing and converges
point-wise to $f$.
...
And now we have the proper machinery.  In particular, a pointwise limit of
measurable functions is again measurable -- so, unlike in the Riemann
theory, the limiting function is never in danger of falling outside the
theory.  We can now work to find conditions
...
It turns out that there is a theorem called ``Lebesgue Dominated Convergence''.
It requires only that the sequence of functions converge ``almost everywhere''
(all but a set of measure $0$) to a function, $f$, and that the sequence be
{\em dominated\/} by a single integrable function $g$; that is,
$|f_n(x)| \le g(x)$ for all $n$ and (almost) all $x$.  Note that the
domination is in {\em absolute value\/} -- bounding the $f_n$ from above
alone is not enough.%
\footnote{For example, the functions $f_n$ equal to $-n$ on $(0, 1/n)$ and
$0$ elsewhere are bounded above by the integrable function $0$ and converge
pointwise to $0$ everywhere, yet $\int_0^1 f_n \, dx = -1$ for every $n$.}
...
be ``dominated'' -- bounded in absolute value -- by a Lebesgue integrable function.
...
in that space. The function space $L^2$ consists of all functions $f$ 
...
product defined, for two functions $f, g \in L^2$ by
...
And, much of the behavior of the function space, $L^2$ is replicated from 
...
working with the vector spaces $Q, Q^2, Q^3, \ldots$, where $Q$ is the set
of rational numbers. The vector properties would be there -- even
perpendicularity is fine -- but the important {\em analytic\/} properties
would be missing: Cauchy sequences need not converge, and nearest points
(orthogonal projections onto closed subspaces) need not exist.

The dictionary of the whole story, in one place:

\begin{center}
\begin{tabular}{l|l}
{\bf Riemann} & {\bf Lebesgue} \\
\hline
partition the {\em domain} & partition the {\em range} \\
step functions & simple functions \\
length of an interval, $|I|$ & measure of a set, $\mu(E)$ \\
upper and lower Darboux sums & supremum over simple minorants \\
uniform convergence & domination $+$ pointwise convergence a.e. \\
Riemann integrable functions & measurable functions \\
the rationals: arithmetic, no limits & the reals: arithmetic {\em and} limits \\
$Q^n$: geometry, no projections & $L^2$: geometry {\em and} projections \\
\end{tabular}
\end{center}

Same game -- integrate by approximating with functions you can already
handle -- on a bigger board: the pieces that changed are listed on the
right.

One of the projections that completeness delivers deserves special mention:
the {\em conditional expectation\/} of probability theory is exactly the
nearest-point (orthogonal) projection of a random variable onto a subspace
of $L^2$. The companion paper ``Measure Theoretic Conditional Expectation
in an Elementary Setting'' works this out concretely in a six-point
discrete world, where the projection is elementary linear algebra.

We should mention that, technically, the function space $L^2$ does not 
...
$a \cdot d = b \cdot c$. Also, note that each equivalence class of integer pairs --
...
\medskip
{\bf Further reading.} Three standard developments of this material:
E. M. Stein and R. Shakarchi, {\it Real Analysis: Measure Theory,
Integration, and Hilbert Spaces\/}; H. L. Royden and P. Fitzpatrick,
{\it Real Analysis\/}; and R. L. Wheeden and A. Zygmund,
{\it Measure and Integral\/}.
