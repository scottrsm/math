...
\begin{align}
	f_n(x) &= \begin{cases}  1 \quad \text{if $x = \frac{p}{q}$ for some $p, q \in {\cal N}$ with $0 \le p \le q \le n$;} \\
...
First, let $\{r_i\}$ be a 1-1 mapping of the positive integers to the
rationals. We know we can do this as the rationals are countable. (Only the
...
Given any $n \in {\cal N}^+$, for any partition of $[0,1]$ we can bound the integral of $f$
...
\begin{align}
	\mu^*(S) &= \inf_{S \subseteq \mathop{\cup}_{i=1}^\infty\limits I_i} \sum_{i=1}^\infty |I_i|
\end{align}
...
intervals -- and also on a finite collection of {\em disjoint\/} intervals
(where it equals the sum of lengths).
...
Then for any
$\epsilon > 0$, $Q_{[0,1]} \subset \mathop{\cup}_{i=1}^\infty\limits \left(r_i - \frac{\epsilon}{2^i}, r_i + \frac{\epsilon}{2^i}\right)$.
