...
in the sense that a set and its ``compliment'' must add up to their union.
When you think about this statement it
...
From this, we can effectively boot-strapconstruct the set of measurable sets.
...
usage to well behaved sets. Unfortunately, to make the restriction precise, 
we made the restriction verification very difficult to use. But fortunately, 
...
a much {\em richer\/} collection of sets automatically satisfy the restriction as 
well and are thus considered measurable sets. If we choose the simple base sets 
to be the collection of open and closed intervals, we have a collection that 
is easy to show satisfy, (\ref{Carthy}).

With a healthy collection of measurable sets we can define ``measurable functions``
as any function that is a simple function or point-wise limits of simple functions.
The Lebesgue integral of these functions is the corresponding finite or infinite
sum of the integrals of their constituent simple functions.
...
much larger class of functions than the set of Riemann integrable functions.
