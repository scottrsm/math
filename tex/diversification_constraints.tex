...
\end{align}
This is $f({\bf x})$ written as an LP.
Although this optimization is succinct, there are still $\binom{n}{k}$ candidates
...
By (\ref{integral}), $\sum_i y^*_i = k$ is an integer; the integer-valued
...
for ${\bf x}$ exactly when $f({\bf x}) \le M$ -- no extra restriction on ${\bf x}$ is introduced.
