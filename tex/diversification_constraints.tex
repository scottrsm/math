...
\section{Overview}
Throughout this paper, $k \in {\bf Z}^+$ with $k \le n$.
...
In finance, this constraint arises when one wants to create (or modify) a
long-only portfolio (${\bf x} \succeq {\bf 0}$) so that its top $k$
securities (notionally) do not exceed a fixed percentage $p$ of its total
notional.  In that case the bound is $M = p \, {\bf 1}^T {\bf x}$, which
depends on the variable ${\bf x}$ --- but only {\it linearly\/}, so the
constraint (\ref{opp1}) below remains linear with $M$ replaced by
$p \, {\bf 1}^T {\bf x}$.  The function $f$ is also $k$ times the
