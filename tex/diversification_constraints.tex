...
\end{eqnarray}
From this we see that $f({\bf x})$ is the value of the sum of the $k$ 
...
write is $n \choose k$. This becomes combinatorially large very quickly.
...
Although this optimization is succinct, there are still combinatorially many
combinations to examine to find the optimal solution in this discrete setting.
...
objective function over a closed, bounded set in ${\bf R}^n$ --
...
By the constraint, (\ref{integral}), the sum of these non-integer components
must be an integer. Consequently, there must be more than one such
...
which by necessity is less than the number of non-integer components in ${\bf y}^*$.
...
${{\bf y}^\prime}^T {\bf 1} = k$. By constraint, (\ref{bound}), the integer
...
The objective value is also preserved: since the ``x'' components on $J \cup {\tilde J}$ all
share a common value, $x_c$, the contribution to the objective from those indices
...
and $\nu \in {\bf R}$, along with the following three constraints:
\begin{eqnarray}
...
optimization. To enforce $f({\bf x}) \le M$ via the primal, we would have to encode
the LP optimality of ${\bf y}^T {\bf x}$ against ${\bf x}$ as a constraint, which
introduces the term $-{\bf y}^T {\bf x}$ from (\ref{op1}). But this term involves
the (now) {\it variable\/}, ${\bf x}$, and the new {\it variable\/}, ${\bf y}$,
in a decidedly {\it non-linear\/} way. In fact, the term is not even {\it convex\/}.
