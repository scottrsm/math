...
\begin{align}
  f({\bf x} + {\bf h}) &= f({\bf x}) + \nabla f({\bf x}) \cdot {\bf h} + o({\bf h}) \label{vderdef2}
\end{align}
...
\begin{align}
  ...
  \lim_{h \rightarrow 0} \left( \frac{f({\bf x} + h {\bf e}_i) - f({\bf x})}{h} + \frac{o(h {\bf e}_i)}{\|h {\bf e}_i\|}\right) \label{vdercomp}
\end{align}
If ${\tilde f}$
...
\begin{align}
  \nabla f({\bf x})_i &=
    ...
    + \lim_{h \rightarrow 0} \frac{o(h {\bf e}_i)}{\|h {\bf e}_i\|}
    \nonumber \\
...
$f$ and $g$. That is, for our function space, define an inner product, $<\!\!<\cdot ,\cdot>\!\!>$ by:
\begin{align*}
...
\begin{align}
  t \, <\!\nabla J(y), h\!> &= t \, <\!w, h\!> + o(t)
\end{align}
Dividing by $t$ and taking $t \rightarrow 0$:
\begin{align}
	<\!\nabla J(y) - w, h\!> &= 0 \label{uniq-t}
\end{align}
...
\left[
  ...
  h(x) \, dx
\end{align}
we claim that with this restriction on $h$, $DJ(y)$ is still uniquely determined.
...
\end{align}
In this case, $F(t, y, y^\prime) = \frac{1}{2} m {y^\prime}^2 - \frac{1}{2} k y^2 $.
