...
When I was a freshman in college I was briefly interested in the idea of 
...
\footnote{Existence and uniqueness of the inscribed circle in a triangle is
a standard fact: the angle bisectors of the triangle are concurrent at the
incenter, which is then equidistant from the three sides.}
...
$90^\circ$; and, as the interior angles of a four-sided {\em convex\/}
...
they have no common integer divisor.}
...
One can solve the quadratic for $r$ (the quadratic formula gives
$r = (a + b \pm c)/2$; only the minus sign is geometrically valid, since
$r < a$ and $r < b$) or read off from the figure
that $(a-r) + (b-r) = c$. Solving this last equation for $r$ gives:
\begin{eqnarray}
  r & = & \frac{a + b - c}{2} \label{radius}
\end{eqnarray}

A {\em primitive Pythagorean triple\/} $(a,b,c)$ is a triple of positive
integers in reduced form satisfying $a^2 + b^2 = c^2$; it gives the side
lengths of a {\em primitive Pythagorean triangle\/}, where $c$ is the
hypotenuse and $a$, $b$ are the legs.
What can we say about the evenness or oddness of the sides when they have no
common factor? Since $a^2 + b^2 = c^2$, taking parities mod $2$ gives
$a^2 + b^2 \equiv c^2 \pmod 2$, so the combined parity of $a, b$ must match
the parity of $c$. This rules out three of the six naive combinations and
leaves:%
...
\end{enumerate}
These equations are to be read: An even integer plus (or minus) an even integer
is an even integer.
...
\begin{itemize}
    \item $c$ is even AND {\em exactly\/} one of the following is true:
        ...
        \end{itemize}
    \item $c$ is odd AND exactly one of $a$ and $b$ is even (the other being odd).
\end{itemize}
Since the triple $(a,b,c)$ is in reduced form, we can eliminate the case of
$a$, $b$, and $c$ all being even. Eliminating that case leaves the following
two possibilities:
\begin{itemize}
...
In both cases $a + b - c$ is even (odd $+$ odd $-$ even,
or odd $+$ even $-$ odd),
so equation \eqref{radius} implies that the radius of the circle, $r$,
is an integer.
Using this fact and equation \eqref{area} we see that $\frac{ab}{2}$ must be
an integer, which means the product $ab$ is even.
This knowledge eliminates the first choice; namely, that $c$ is even with
both $a$ and $b$ odd. Consequently, $c$ is odd, with exactly one of $a$ and $b$ even
(the other being odd).

The whole elimination tournament can now be seen at a glance. Writing E
for even and O for odd, there are six possible parity patterns for
$(a, b, c)$ (up to a swap of the legs -- we write the mixed-leg cases with
$b$ even); each losing pattern is eliminated by a specific weapon, and
exactly one pattern survives:
\begin{center}
\begin{tabular}{ccc|l}
$a$ & $b$ & $c$ & fate \\
\hline
E & E & O & eliminated: parities must match in $a^2 + b^2 = c^2$ \\
O & O & O & eliminated: parities must match in $a^2 + b^2 = c^2$ \\
O & E & E & eliminated: parities must match in $a^2 + b^2 = c^2$ \\
E & E & E & eliminated: the triple is in reduced form \\
O & O & E & eliminated: the area equation forces $ab$ even \\
O & E & O & {\bf survives} --- the only possibility \\
\end{tabular}
\end{center}

Following the surviving row, we assume (without loss of generality) that
$b$ is even; in which case, $a$ is odd.

Now, the left hand side of \eqref{area} simplifies to
$r(a + b - r) = r(r + c)$ by \eqref{radius}; with $c$ odd, this is even
regardless of $r$'s parity. This implies that $\frac{ab}{2}$
...
Let $(a, b, c)$ be a primitive Pythagorean triple. Then:
\begin{itemize}
  \item One of the legs is odd; the other leg is even; and the hypotenuse
      is odd.
  \item The even leg is divisible by $4$.
  \item The radius, $r = (a + b - c)/2$, of the inscribed circle in the
      associated Pythagorean triangle is an integer (necessarily positive).
\end{itemize}
\end{proposition}

{\bf Remark (what is ``alternative'' here).} The standard routes to the
first two facts are arithmetic. One goes through Euclid's parametrization
of primitive triples, $(a, b, c) = (m^2 - n^2,\, 2mn,\, m^2 + n^2)$ with
$m > n$ coprime and of opposite parity, from which divisibility of the
even leg by $4$ is immediate (in $2mn$, one of $m$ and $n$ is even).
Another works modulo $8$: odd squares are congruent to $1$ mod $8$, so
with $a$ and $c$ odd, $b^2 = c^2 - a^2 \equiv 0 \pmod 8$, which forces
$4 \mid b$. The proof above replaces both with a single strategic move ---
{\em compute one quantity two ways\/}. The area of the triangle, reckoned
whole and reckoned piece by piece, changes the representation of the
algebraic identity $a^2 + b^2 = c^2$ into the integrality of the inradius
$r = (a + b - c)/2$, and elementary even/odd bookkeeping does the rest.
The dividend of the geometric route is the third conclusion --- the
integer inradius --- which the arithmetic proofs never see, because their
machinery never draws the circle.

The freshman project paid off after all: with no machinery beyond a
dissected figure and parity arithmetic, the area accounting reproduced two
classical theorems and added a third fact for free. And the story
continues from here. Knowing now that the hypotenuse of a primitive triple
must be odd, one can ask {\em which\/} odd numbers actually occur as
hypotenuses, in how many ways, and how the prime hypotenuses are
distributed among the primes. Those questions --- one of which remains
open --- are the subject of the companion note,
{\em A Conjecture on Primitive Pythagorean Triples\/}
(\texttt{tex/conj\_pythag\_triple.tex} in this repository).
