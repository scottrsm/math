...
\footnote{Existence and uniqueness of the inscribed circle in a right triangle is
a standard fact: the angle bisectors are concurrent at the incenter, which lies
at distance $r = (a+b-c)/2$ from each side. With the right-angle vertex at $(a,0)$
and the legs along $y=0$ and $x=a$, the incenter has coordinates $(a-r, r)$.}
...
\begin{itemize}
    ...
    \item $c$ is odd AND {\em exactly\/} one of the following is true:
        \begin{itemize}
            \item Exactly one of $a$ and $b$ is even (the other being odd).
        \end{itemize}
\end{itemize}
Since the triple $(a,b,c)$ is in reduced form, we can eliminate the case
of all three being even.
By the Pythagorean theorem we know that $a^2 + b^2 = c^2$. Using this we can 
...
Regardless of which choice occurs, equation \eqref{radius} implies that 
