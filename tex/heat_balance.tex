...
Heat a small spot on a metal bar and the hot spot spreads and smooths.
Drop a speck of dye into still water and it does the same. Now let a
particle be carried along by a known differential equation while being
jostled at random: our knowledge of where it is --- a probability
density --- spreads and smooths in just the same way. Why do all three
spread by the same law? Because all three are governed by the same
piece of bookkeeping, a {\it balance law\/}: whatever accumulates in a
region must have crossed its boundary.

In this paper we play this one game on ever larger boards. First we
use a balance law to derive the heat equation, in one and then in
several dimensions. Next we use the same balance law --- with
probability in place of heat, and with regions that move with the flow
of a differential equation --- to derive the equation governing the
propagation of a probability density function driven by that
differential equation. Finally, we observe that a balance law is
linear in the flux, so fluxes add: crediting a moving region with both
mechanisms of transfer at once --- probability carried by the flow,
and probability diffusing through the boundary as heat does ---
...
The derivation is a four step template: balance the quantity of
interest in an arbitrary region over an arbitrary time interval;
shrink the time interval to get an instantaneous balance; use the
arbitrariness of the region to localize --- converting an integral
statement into a pointwise one; and close with an assumption relating
the flux to the quantity being balanced. Every equation in this paper
--- the Fokker-Planck equation included --- comes from replaying
these four steps on a bigger board.


\subsection{The Heat Equation in One Dimension}
We derive a partial differential equation for the temperature distribution
in a one dimensional material. The reader has likely already solved the
lumped version of this problem: Newton's law of cooling, in which the whole
object has a single temperature $T(t)$ satisfying $\dot T = -r(T - T_a)$.
The heat equation is what replaces Newton's law when the temperature is
allowed to vary from point to point, so that the object must do its
bookkeeping locally. To do this we
...
\footnote{\kern -2pt \raise 0.5ex \hbox{\dag}}{Positive heat flux $f$ means
heat flowing in the direction of increasing $x$.}. The increase (or decrease) in 
the heat of 
...
$$
Now, divide this expression by $(t_2 - t_1)$ and take the
...
By the {\it Fundamental Theorem of Calculus\/}, 
...
proportional to temperature:\footnote{\raise 0.5ex \hbox{\dag}}{%
The temperature of a substance is the average kinetic energy of the
material's molecules. When we refer to heat we mean the heat density, the
amount of heat energy per unit volume. The constant $c$ used here is the
{\it volumetric\/} heat capacity (energy per unit volume per kelvin),
equal to $\rho \, c_p$ where $\rho$ is the mass density and $c_p$ is the
specific heat per unit mass. We will follow common practice and refer to
$c$ as the specific heat, with this caveat understood.}
$h(t,x) = c \, u(t,x)$.
...
$$
\fracd{\partial u}{\partial t} = \kappa \fracd{\partial^2 u}{\partial x^2}.
$$
...
their role. Consider a bar made of differing materials.
With differing materials we have two independent mechanisms controlling heat flow:
the heat capacity of any portion of the bar controls the value of the
specific heat, $c$; and, the insulating property of the material controls the heat flux rate, $k$.

Under these conditions, we can get a better physical
understanding of the influence
of $c$ and $k$ on $\kappa$. If the bar behaves more like water at a certain
spot, it can absorb more heat and therefore is less diffusive there.
If the bar is made of a material that is more
insulating at a certain spot, like styrofoam, the heat flux rate constant
goes down and therefore the material is less diffusive there. (Varying
cross-sectional thickness manifests in the 1-D model only through its
effect on $c$ and $k$.)
...
\hbox{$\Omega$}}} \; dt
$$
As in the one dimensional case, divide by $(t_2 - t_1)$
...
$$
If we {\it assume\/} that the integrand is continuous, then since the
region $\Omega$ is arbitrary, the integrand must vanish for all ${\bf x}$;
that is,
$$
...
When $K({\bf x})$ is a matrix, the heat flux vector ${\bf f}(t,{\bf x})$ will not,
...
{\bf x}(0) = {\bf x}_0  & & (5b) \cr }
$$
let ${\cal X}(t; {\bf x}_0)$ be the solution; 
which is assumed to be invertible 
on the interval $[0,T)$.
That is, for each $t \in [0,T)$, ${\cal X}(t; \cdot): R^n \to R^n$
...
${\cal X}(t; \cdot)$ to the random variable $X$.
...
= 0 & (6) \cr}
$$
The new difficulty, compared with the heat sections, is that the region
itself moves. We handle it by changing the representation: change the
integration variable so as to keep the region constant, differentiate,
and change back. To this end, let
${\bf x} = {\cal X}(\tau, {\bf z})$. The integral becomes:
$$
\fracd{d}{d\tau} \int_{\Omega}
p(\tau,{\cal X}(\tau, {\bf z})) \bigg|
...
as\footnote{\kern 1pt \raise 0.5ex \hbox{\dag}}{This is Liouville's
formula, and it is four lines from tools we already have. First,
differentiating (5a) with respect to ${\bf z}$ gives
$\frac{d}{d\tau} \frac{\partial {\cal X}}{\partial {\bf z}} =
\nabla {\bf f}(\tau, {\cal X}(\tau, {\bf z})) \,
\frac{\partial {\cal X}}{\partial {\bf z}}$.
Second, Jacobi's formula for differentiating a determinant states
$\frac{d}{d\tau} \det M = \det M \; {\rm tr}\big(M^{-1}
\frac{dM}{d\tau}\big)$. Applying Jacobi's formula with
$M = \frac{\partial {\cal X}}{\partial {\bf z}}$ and substituting the
first fact, the factors $M$ and $M^{-1}$ cancel inside the trace,
leaving ${\rm tr}\big(\nabla {\bf f}\big)$.}
$$
\frac{d}{d\tau} 
\bigg| \frac{\partial {\cal X}(\tau,{\bf z})}
...
$$
\eqalignno{
\int_{\Omega} \bigg( p_\tau(\tau, {\cal X}(\tau, {\bf z})) +
...
Assuming the integrand in (9) is continuous, and since
$\widehat \Omega = \Omega_t$ and $t$ are both arbitrary, we have for all
$t \in [0,T)$ and for all ${\bf x}$
$$
\eqalignno{
p_t(t, {\bf x}) + \nabla \cdot 
...
\big( p(t, {\bf x}) f_i(t, {\bf x})\big) & = 0  \cr
p(0, {\bf x})  = P_X({\bf x})&  \cr} 
$$
Equations (10a-b) constitute a first-order initial-value problem
...
{\bf x}(0) = {\bf x}_0 & & (11b)  \cr
\dot {\bf x}(0) = {\bf v}_0 & & (11c)  \cr} 
$$
...
{\bf x}(0)  & = {\bf x}_0  & (12c) \cr 
{\bf v}(0)  & = {\bf v}_0  & (12d) \cr }
$$
...
Let $p(t,{\bf x},{\bf v}) =
P_{{\cal X}(t; X, V), {\cal V}(t; X, V)}({\bf x}, {\bf v})$
denote the probability density function of the pair $(X, V)$, transformed by
the solution of (12a-d) at time $t$ (here we evaluate the deterministic flow at the
random initial conditions, producing random variables ${\cal X}(t; X, V)$ and
${\cal V}(t; X, V)$ with joint density $p$).
...
+ \varphi \, \nabla \cdot {\bf w}$ to combine the last two terms,
$$
\eqalignno{
p_t(t, {\bf x}, {\bf v}) + \nabla_{\bf x}p(t, {\bf x}, {\bf v})
\cdot {\bf v} +
...
equation. Note that $p(t, {\bf x})$ can no longer be the transported
density of the last section: for that density, equation (6) says the
probability in a moving region never changes, which is exactly what
diffusion violates. Instead, let $X$ be a random variable with density
$P_X({\bf x})$, let ${\cal X}(t; {\bf x}_0)$ be the solution of (5a-b),
and let $p(t, {\bf x})$ denote the (as yet unknown) probability density
of the state at time $t$, whose primary dynamics is the flow of (5a-b).
We replace assumption (6): a moving region now also gains or loses
probability by diffusion through its boundary. As before,
...
The computation (7)--(9) of the last section nowhere used the fact that
$p$ was the transported density: it is an identity, valid for any smooth
density $p$, expressing the derivative of the left hand side of (16) when
the regions $\Omega_\tau$ move with the flow of (5a-b). Reusing it, we
may write
$$
\eqalignno{
\int_{\Omega_\tau} p_\tau(\tau, {\bf x}) + \nabla \cdot 
...
$$
Using the {\it Divergence Theorem\/} on the right hand side
...
\sum_{i=1}^n \fracd{\partial}{\partial x_i} 
\bigg(
\sum_{j=1}^n A_{ij}({\bf x}) \fracd{\partial p(t, {\bf x})}{\partial x_j}
\bigg)
& (20a) \cr
p(0, {\bf x}) = P_X({\bf x}) & & (20b) \cr }
$$
A remark on the form of the diffusion term is in order. The balance
law (16) models the diffusive flux as $-A({\bf x}) \nabla p$ ---
Fick's law, exactly as in the heat equation --- which produces the
diffusion term $\nabla \cdot \big(A({\bf x}) \nabla p\big)$. The
Fokker-Planck equation associated with a stochastic differential
equation is often written instead with diffusion term
$\sum_{i,j} \fracd{\partial^2}{\partial x_i \partial x_j}
\big(D_{ij}({\bf x}) \, p\big)$.
The two forms agree when the diffusion matrix is constant; when it
varies with ${\bf x}$ they differ by a term of first order, which may
be absorbed into the drift ${\bf f}$. The form derived here is the one
...
If $X$ and $V$ are random variables with joint density
$P_{X,V}({\bf x}, {\bf v})$, and
$\{{\cal X}(t; {\bf x}_0, {\bf v}_0)~,~{\cal V}(t; {\bf x}_0, {\bf v}_0)\}$
is the pair of functions representing the solution of (12a-d), then ---
as in the last section --- let $p(t, {\bf x}, {\bf v})$ denote the
(as yet unknown) joint probability density of the state
$({\bf x}, {\bf v})$ at time $t$, whose primary dynamics is the
flow of (12a-d).
As before, let $\widehat \Omega$ be an arbitrary region in $R^n \times R^n$, 
and let 
$\Omega_\tau$ parameterize a family 
of regions in $R^n \times R^n$ such that 
...
A({\bf x}, {\bf v}) \nabla_{\bf v}
p(\tau, {\bf x}, {\bf v})} \cdot \pmatrix{{\bf n}_x \cr
{\bf n}_v} \; d{\bf S} & (21) \cr}
$$
Here $({\bf n}_x, {\bf n}_v)$ is the outward normal to the boundary of
$\Omega_\tau$, a surface in $R^n \times R^n$ with surface measure
$d{\bf S}$.
The left hand side has been previously calculated; so, using the
{\it Divergence Theorem\/} on the right hand side, we may
write
$$
\eqalignno{
...
p(\tau, {\bf x}, {\bf v})\big) \; d{\bf x} \, d{\bf v}
 & = 0 & (22) \cr}
$$
Again, since $\widehat \Omega = \Omega_t$ and $t$ are arbitrary, the
...
$$
The coordinates for the right-hand side were written in the last section,
so we may write the coordinate version of this Fokker-Planck equation as
$$
\eqalignno{
\fracd{\partial p(t, {\bf x}, {\bf v})}{\partial t} +
...
& = 
\sum_{i=1}^n \fracd{\partial}{\partial v_i} 
\bigg(\sum_{j=1}^n A_{ij}({\bf x}, {\bf v})
...
$$
If we further specialize ${\cal A}$ to the identity matrix, then (25a-b) becomes
$$
\eqalignno{
\fracd{\partial p(t, {\bf x}, {\bf v})}{\partial t} +
\sum_{i=1}^n \fracd{\partial p(t, {\bf x}, {\bf v})}{\partial x_i}
...
& = \kappa 
\sum_{i=1}^n 
\fracd{\partial^2 p(t, {\bf x}, {\bf v})}{\partial v_i^2}
  & (26a) \cr
...
\kappa \fracd{\partial^2 p(t, x, v)} {\partial v^2} & (27a) \cr
p(0, x, v) = P_{X,V}(x, v) & & (27b) \cr }
$$

\section{One Game, Five Boards}

Every equation in this paper came from the same four steps: balance a
quantity in an arbitrary region; shrink the time interval to get an
instantaneous balance; use the arbitrariness of the region to
localize; and close with an assumption about the flux. Only the board
changed.

\centerline{\vbox{\halign{\quad#\hfil\quad&\quad#\hfil\quad&\quad#\hfil\quad\cr
{\bf Board} & {\bf Flux through the boundary} & {\bf Resulting equation}\cr
\noalign{\smallskip\hrule\smallskip}
interval $[x_1, x_2]$ & $-k u_x$ & $u_t = \kappa u_{xx}$\cr
region $\Omega \subset R^n$ & $-K \nabla u$ & $u_t = \kappa \, \Delta u$\cr
moving region $\Omega_\tau$ & none: the region rides the flow &
$p_t + \nabla \cdot (p {\bf f}) = 0$\cr
moving region $\Omega_\tau$ & $-A \nabla p$ &
$p_t + \nabla \cdot (p {\bf f}) = \nabla \cdot (A \nabla p)$\cr
phase space $({\bf x}, {\bf v})$ & $-A \nabla_{\bf v} p$, in ${\bf v}$ only &
equation (23a)\cr
}}}

This table answers the question posed in the Overview. The hot spot on
the bar, the speck of dye, and the randomly jostled particle spread by
the same law because they are governed by the same bookkeeping:
whatever accumulates in a region must have crossed its boundary. And
the Fokker-Planck equation, which can appear as a pronouncement when
first met, is from this point of view not a new kind of equation at
all. A balance law is linear in the flux, so fluxes add: credit a
moving region with the flow's transport and with a Fickian diffusion
through its boundary, and the Fokker-Planck equation is what the four
steps produce. The reader who can replay the four steps of Section 2
owns every equation in this paper.
\bye
