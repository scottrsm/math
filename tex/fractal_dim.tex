...
\mantitle{Defining Fractal Dimension}{R. Scott McIntire}{May 2, 2026}
...
all real (and complex) arguments and satisfies $\Gamma(z+1) = z \Gamma(z)$
for all $z \in R^+$. On the positive integers it agrees with the factorial:
...
by: $R^+ = \{x \, | \, x\in R \; \wedge \; x > 0\}$. The formula has poles
at $\alpha = 0$ (from $1/\alpha$ and $\Gamma(\alpha/2)$), so we exclude
$\alpha = 0$ from $R^+$ throughout.}
$$
...
be finite and that each have diameter no larger than $\epsilon$.\ftn{This is the
standard Hausdorff‑measure formulation: as $\epsilon$ shrinks, the
allowed covers must be finer, so fewer covers are admitted and the
infimum can only grow. The construction is a covering analogue of an
outer measure.}
...
threshold value of $\alpha$: above it the $\alpha$-dimensional volume is
zero, below it the volume is infinite (and at $\alpha = \alpha^*$ itself
the volume is typically a finite positive number, the fractal volume).
...
as $n \to \infty$, so ${\cal V}_\alpha(L) = 0$. For $\alpha < 1$ the
expression diverges as $n \to \infty$, so ${\cal V}_\alpha(L) = +\infty$.
The threshold value is $\alpha = 1$, where the term is identically $1$ and
...
($\frac{2}{3^\alpha} > 1$), and a more careful argument (using the
self-similarity of $C$) shows that no cover can do better, so
${\cal V}_\alpha(C) = +\infty$ in that range.
...
\pi^{\alpha^*/2}}{\alpha^* \Gamma(\alpha^* / 2)} \approx 1.03505$, and by
self-similarity this bound is in fact attained, so
${\cal V}_{\alpha^*}(C) \approx 1.03505$.
