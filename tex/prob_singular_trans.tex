...
$\langle(P^TP  - I){\bf x}, {\bf y}\rangle \; = \; 0 \enspace \forall {\bf x}, {\bf y} \in {\bf R}^n$. 
...
$PA$ is a map from ${\bf R}^n$ to ${\bf R}^n$, so it changes the volume of the unit cube
by $|PA|$. Since for given transformations, $B, C: {\bf R}^n \to {\bf R}^n$ we have 
...
$$
\eqalignno{
	P_Y(y) & = \frac{e^{-y/(2\sigma^2)}}{(2\pi)^{n/2} \sigma^n 2\sqrt{y}} \int_{\partial B(\sqrt{y})} d\, {\bf x}& \cr
}
$$
