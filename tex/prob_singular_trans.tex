...
$\{ {\bf x}_p + {\bf x}_n \mid {\bf x}_n \in N(A)\}$. The pseudo inverse of ${\bf y}$
is defined to be ${\bf x}_p$. Since $R(A^T) \perp N(A)$, ${\bf x}_p$ is the 
...
\section{Proof of Singular Transformation Formula}
...
\eqalignno{
\nu(\Omega_Y) & =
\int_{\Omega_X} P_X({\bf x}) \, d{\bf x} & (2) \cr
}
$$
...
locally, ${\bf x}_1 \approx Df^T({\bf x}) (Df({\bf x}) Df^T({\bf x}))^{-1} {\bf y}$.
...
We use the unparameterized form of the transformation formula, equation~(3),
applied with $\Omega_{X_2} = f^{-1}(y)$.}
$$
\eqalignno{
