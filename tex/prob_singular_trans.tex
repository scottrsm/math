...
respect to Lebegue measure\footnote{\ddag}{Since the Lebegue mesaure is regular, any Lebegue measureable set can be approximated by a Borel measureable set.} 
so that there exists a mesureable function, $P_Y(y)$
...
$$
Since this is true for all Borel measureable sets, $\Omega_Y$, we have
$$
P_Y({\bf y}) =
\int_{\Omega_{X_2}} 
{P_X({\bf x}({\bf y}, {\bf x_2})) \over 
...
In this case we add theor contributions to the distribution of the target variable, $Y$.
